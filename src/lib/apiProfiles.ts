...
export function isDefaultConfigOnlyEnabled(): boolean {
  return SHOW_DEFAULT_CONFIG_ONLY && hasDefaultPresetConfig()
}
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  const profiles = syncDefaultTextProfileApiKey(normalizedProfiles.map((profile) => ({
    ...profile,
...
function hasOnlyDefaultProfiles(settings: AppSettings): boolean {
  ...
  return Boolean(imageProfile && textProfile &&
    ...
    imageProfile.transparentBackgroundMethod === DEFAULT_IMAGE_PROFILE.transparentBackgroundMethod &&
    ...
    textProfile.transparentBackgroundMethod === DEFAULT_TEXT_PROFILE.transparentBackgroundMethod)
}
