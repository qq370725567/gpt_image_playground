...
| 功能                              | 说明                                                                                                                                                           | 相对原版的优势                           |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| 🔑 **API Key 配置引导**           | 首次使用弹出引导弹窗，快速配置 API Key，并且自动从 sub2api 站点识别用户创建的 API Key                                                                          | 原版需手动进入高级设置，上手门槛高       |
| 🔀 **图片/文本 API Key 独立设置** | 简易配置弹窗可开启「分别设置图像模型和文本模型的 API Key」，两个独立输入框 + 各自 sub2api 下拉                                                                 | 图像和文本支持分开独立设置不同的 API Key |
| 🎛️ **模型选择面板**               | 右下角悬浮面板，可切换 Agent 文本模型（`gpt-5.6-sol` 旗舰 / `gpt-5.6-terra` 均衡 / `gpt-5.6-luna` 轻量），悬停可查看各模型能力说明；图像模型固定 `gpt-image-2` | 原版文本模型需要自己输入，不能选择       |
| 🌐 **文本模型前缀可配置**         | 环境变量 `VITE_DEFAULT_TEXT_MODEL_PREFIX`（如 `openai` → 请求发送 `openai/gpt-5.6-sol`；不设置则发送裸模型名）                                                 | 原版无法设置前缀                         |
...
- **多服务商接入**：内置 OpenAI 兼容接口（含 `Images API` 和 `Responses API`）、sub2api（异步）、fal.ai（支持队列），并支持通过 JSON 导入自定义 HTTP 服务商配置（兼容同步/异步任务）。
