...
| 🔀 **图片/文本 API Key 独立设置** | 简易配置弹窗可开启「分别设置图像模型和文本模型的 API Key」，两个独立输入框 + 各自 sub2api 下拉                                                                 | 原版两模型强制共用同一 Key，无法使用不同服务商                       |
...
| 🌐 **文本模型前缀可配置**         | 环境变量 `VITE_DEFAULT_TEXT_MODEL_PREFIX`（如 `openai` → 请求发送 `openai/gpt-5.6-sol`；不设置则发送裸模型名）                                                 | 原版前缀写死，无法兼容要求裸模型名的服务商，适配 Vercel 代理路由约定 |
...
_(注：使用 host 网络时加 `--network host`，修改容器监听端口使用 `-e PORT=28080`)_
